...
## 1. 目的
tracker-bridge は、外部トラッカー（Jira / GitHub Issues / Backlog / Linear など）と、内部の `workx` / `memx-core` を接続するための同期ブリッジである。
...
## 2. 背景
`memx-core` は知識・証拠・要約を扱う記憶基盤であり、`workx` は task / decision / open_question / context_bundle など内部作業状態を扱う。
...
- `workx`: 内部 task / decision / question / run の正本
...
### FR-03 Entity Link
- 外部 issue と内部 `workx.task` を対応付けできること
...
### NFR-04 疎結合
`workx` / `memx-core` と DB レベルで密結合しないこと。
...
3. issue と `workx.task` の entity_link 作成
